...
model =TransformerDMS( in_channels, input_features_enc, input_features_dec, nheads_enc, nheads_dec,
                 embedding_features, output_features, dim_feedforward_enc, dim_feedforward_dec,n_enc_layer, n_dec_layer,input_length, output_length)
#模型调用
model.to(device)
...
for epoch in range(epochs):
    ...
    for step, (feature_, label_,total_labels) in enumerate(train_Loader):
        ...
        loss.backward()
        # torch.nn.utils.clip_grad_norm(seq2seq_model.parameters(), 0.15)
        optimizer.step()
    ...
    model.eval()
    val_loss_sum = 0
    combined_val_loss_sum = 0  # 初始化 combined_output 的损失累加器
    ...
    for val_step, (feature_, label_,total_labels) in enumerate(val_Loader):
        with torch.no_grad():
            ...
            val_loss_a = loss_func(combined_output, total_labels)  # 计算 combined_output 的损失
            val_loss = val_loss_c + val_loss_a  # 可能会考虑对这两个损失进行加权
        val_loss_sum += val_loss_c.item()
        combined_val_loss_sum += val_loss_a.item()  # 更新 combined_output 的损失
    # 更新模型保存逻辑以依赖于 combined_output 的表现
    if epoch == 0 or val_best > combined_val_loss_sum:
        val_best = combined_val_loss_sum
        print('val_loss = ' + str(val_loss_sum) + ", combined_val_loss = " + str(combined_val_loss_sum))
        torch.save(model.state_dict(), f'./weighting/model_Parformer_weights_A_{output_length}')  # 保存最好权重
        if epoch != 0:
...
for step, (feature_, label_, total_labels) in enumerate(test_Loader):
    with torch.no_grad():
        prediction, combined_output = model(feature_)  # 确保接收两个输出
        if step == 0:
            pre_array = prediction.cpu()
            combined_array = combined_output.cpu()  # 跟踪 combined_output
            label_array = label_.cpu()
            total_labels_array = total_labels.cpu()  # 可能需要这个如果total_labels是tensor

            loss_c = loss_func(prediction, label_)
            loss_a = loss_func(combined_output, total_labels)  # combined_output 的损失
            test_loss_sum += loss_c.item()
            combined_test_loss_sum += loss_a.item()  # 更新 combined_output 的损失
        else:
            pre_array = np.vstack((pre_array, prediction.cpu()))
            combined_array = np.vstack((combined_array, combined_output.cpu()))  # 更新 combined_output 数组
            label_array = np.vstack((label_array, label_.cpu()))
            total_labels_array = np.vstack((total_labels_array, total_labels.cpu()))  # 更新 total_labels 数组
...
# 假设 combined_array 和 total_labels_array 是你的两个数组
# 确保它们的形状是一致的
...
epsilon = 1e-8
relative_error = np.mean(np.abs((total_labels_array - combined_array) / (total_labels_array + epsilon))) * 100
print(f"Relative Error (RE): {relative_error:.2f}%")
